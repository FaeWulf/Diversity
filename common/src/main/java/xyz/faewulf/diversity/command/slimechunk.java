...
public class slimechunk {
    ...
    static private int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        ChunkPos chunkPos = new ChunkPos(serverPlayerEntity.blockPosition().getX(), serverPlayerEntity.blockPosition().getZ());
        ...
        if (isSlimeChunk)
            serverPlayerEntity.sendOverlayMessage(Component.literal("You are standing in the slime chunk").withStyle(ChatFormatting.GREEN));
        else
            serverPlayerEntity.sendOverlayMessage(Component.literal("You are not standing in the slime chunk").withStyle(ChatFormatting.GRAY));
