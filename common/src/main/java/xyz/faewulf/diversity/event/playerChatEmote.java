...
public class playerChatEmote {
    public static void run(PlayerChatMessage message, ServerPlayer sender, ChatType.Bound params) {
        ...
        if (content.contains("meow") || content.contains(":3"))
            soundShouldPLay.add(SoundEvents.CAT_SOUNDS.get(CatSoundVariants.SoundSet.CLASSIC).adultSounds().ambientSound().value());
        if (content.contains("purr"))
            soundShouldPLay.add(SoundEvents.CAT_SOUNDS.get(CatSoundVariants.SoundSet.CLASSIC).adultSounds().purrSound().value());
        if (content.contains("purroew"))
            soundShouldPLay.add(SoundEvents.CAT_SOUNDS.get(CatSoundVariants.SoundSet.CLASSIC).adultSounds().purreowSound().value());
        if (content.contains("woof"))
            soundShouldPLay.add(SoundEvents.WOLF_SOUNDS.get(WolfSoundVariants.SoundSet.CLASSIC).adultSounds().ambientSound().value());
