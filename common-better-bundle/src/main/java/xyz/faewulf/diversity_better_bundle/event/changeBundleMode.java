package xyz.faewulf.diversity_better_bundle.event;
            ...
            if (hand == InteractionHand.MAIN_HAND) {
                ...
                if (!stack.isEmpty()) {
                    if (stack.getItem() instanceof BundleItem bundleItem && serverPlayer.isShiftKeyDown()) {
                        ...
                        int mode = ((ICustomBundleItem) bundleItem).multiLoader_1_20_1$getMode(stack);

                        mode++;
                        if (mode > 2)
                            mode = 0;

                        ((ICustomBundleItem) bundleItem).multiLoader_1_20_1$setMode(stack, mode);
